...
class TestBuildDifferenceSummary(unittest.TestCase):
    ...
    def test_select_list_differences(self):
        test_cases = [
            (
                "items missing",
                "SELECT id, name FROM users",
                "SELECT id FROM users",
                "SELECT list differs: items only in SQL1: 1",
                True
            ),
            (
                "items added",
                "SELECT id FROM users",
                "SELECT id, name FROM users",
                "SELECT list differs: items only in SQL2: 1",
                False
            ),
            (
                "order differs",
                ...
                "SELECT list order differs (same items, different order).",
                False
            ),
        ]

        for msg, norm_a, norm_b, expected_summary, expect_token_changes in test_cases:
            with self.subTest(msg=msg):
                ...
                if expect_token_changes:
                    self.assertTrue(any("Token-level changes" in s for s in summary))

    def test_where_and_terms_differences(self):
        test_cases = [
            (
                "terms missing",
                "SELECT id FROM users WHERE active = 1 AND role = 'admin'",
                "SELECT id FROM users WHERE active = 1",
                "WHERE AND terms differ: terms only in SQL1: 1",
            ),
            (
                "terms added",
                "SELECT id FROM users WHERE active = 1",
                "SELECT id FROM users WHERE active = 1 AND role = 'admin'",
                "WHERE AND terms differ: terms only in SQL2: 1",
            ),
            (
                "order differs",
                "SELECT id FROM users WHERE active = 1 AND role = 'admin'",
                "SELECT id FROM users WHERE role = 'admin' AND active = 1",
                "WHERE AND term order differs (same terms, different order).",
            ),
        ]

        for msg, norm_a, norm_b, expected_summary in test_cases:
            with self.subTest(msg=msg):
                tokens_a = tokenize(norm_a)
                tokens_b = tokenize(norm_b)
                summary = build_difference_summary(
    ...
    def test_join_analysis_differences(self):
        test_cases = [
            (
                "reorderable components missing",
                "SELECT id FROM users JOIN roles ON users.role_id = roles.id JOIN permissions ON roles.id = permissions.role_id",
                "SELECT id FROM users JOIN roles ON users.role_id = roles.id",
                "Reorderable JOIN components differ: 1 only in SQL1.",
            ),
            (
                "reorderable components added",
                "SELECT id FROM users JOIN roles ON users.role_id = roles.id",
                "SELECT id FROM users JOIN roles ON users.role_id = roles.id JOIN permissions ON roles.id = permissions.role_id",
                "Reorderable JOIN components differ: 1 only in SQL2.",
            ),
            (
                "reorderable segments order differs",
                "SELECT id FROM users JOIN roles ON users.role_id = roles.id JOIN permissions ON roles.id = permissions.role_id",
                "SELECT id FROM users JOIN permissions ON roles.id = permissions.role_id JOIN roles ON users.role_id = roles.id",
                "Reorderable JOIN segment order differs (same components, different order).",
            ),
        ]

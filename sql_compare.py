...
class _SQLStateScanner:
    __slots__ = ('sql', 'n', 'i', 'mode', 'level')
    ...
    def advance(self):
        # Process one token/char. Returns (consumed_text, mode_before).
        if self.i >= self.n:
            return '', self.mode
...
def uppercase_outside_quotes(s: str) -> str:
    ...
    """
    out = []
    scanner = _SQLStateScanner(s)
    while scanner.i < scanner.n:
        text, mode = scanner.advance()
        if mode is None:
...
def remove_outer_parentheses(s: str) -> str:
    ...
    def is_wrapped(text: str) -> bool:
        if not (text.startswith("(") and text.endswith(")")):
            return False
        scanner = _SQLStateScanner(text)
        while scanner.i < scanner.n:
            start_level = scanner.level
            text_chunk, _ = scanner.advance()
            # If level dropped to 0 and we are NOT at the end
            if start_level > 0 and scanner.level == 0 and scanner.i < len(text):
                return False
        return True
